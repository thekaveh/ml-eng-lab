...
As of genai-vanilla `ba21661` (pinned in `vendor/genai-vanilla`), the `jupyterhub` image natively ships the ml-eng-lab dependency set — `thekaveh-nnx[lm]==0.2.0`, `python-louvain`, `nltk`, `spacy`, `torchao`, and `prettytable` — plus the `en_core_web_sm` spaCy model and `vader_lexicon` NLTK corpus baked at image-build time. Two paths, pick by need.
