...
The recommended runtime for these notebooks is the `jupyterhub` service in the [`genai-vanilla`](https://github.com/thekaveh/genai-vanilla) stack. As of genai-vanilla `a22b182` (pinned in this repo's `vendor/genai-vanilla` submodule), that image natively ships the ml-eng-lab dependency set:
...
genai-vanilla `a22b182a0f0cd1bb0be3599a7710d87890491eb8`, which includes the ml-eng-lab runtime dependency block, `thekaveh-nnx[lm]==0.2.0`, and the spaCy/NLTK asset downloads in the JupyterHub image.

## 6. Common failure modes

- **`Could not find a version that satisfies the requirement nnx-pytorch`** during `docker compose build jupyterhub` — the checkout is older than the `a22b182` runtime pin. Pull current genai-vanilla `main` or update this repo's submodule with `git submodule update --init --recursive`.
