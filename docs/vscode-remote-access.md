...
Use this when you want the full container shell experience or when you're hacking on nnx via the editable-install override.
...
**Coverage:** This works out of the box for **26 of the 29 ml-lab notebooks**. Three exceptions: (a) `image_classification-mnist-ffnn-numpy/notebook.ipynb`, which imports 8 sibling `.py` modules from its own task folder (`consts`, `feed_fwd_nn`, `linear_layer`, etc.) that aren't pip-installable and require the ml-lab repo to be accessible inside the container; (b) `text_generation-tinyshakespeare-transformer-pytorch/notebook.ipynb` (`NNTokenizerParams` at cell 6) and (c) `preference_alignment-toy-dpo-pytorch/notebook.ipynb` (`train_bpe`/`NNTokenizerParams` at cell 10), which both need the `nnx[lm]` extra — the standalone genai-vanilla image currently bakes `nnx-pytorch` without extras, so those imports fail on this Mode-2 path; bumps to 28/29 once the upstream image picks up `nnx-pytorch[lm]` (tracked as a follow-up to issue #12). For the numpy notebook, use the §2-wrapper path of [jupyterhub-integration.md](jupyterhub-integration.md#2-persistence-path-wrapper-script--bind-mount) (bind-mount the repo) and open the notebook from `/home/jovyan/work/ml-lab/image_classification-mnist-ffnn-numpy/notebook.ipynb`.
