...
The CodeXGLUE notebooks now resolve paths relative to their archived folders and skip
evaluation/preview cells when the historical `repos/CodeXGLUE` checkout, checkpoints,
datasets, or `model/test_0.*` files are absent. The RoBERTa-family notebooks also keep
historical `model/` artifacts intact by default; set `ARCHIVE_RERUN_ENABLED = True` inside a
notebook only when you intentionally want to clone/download/train/test and overwrite archive
outputs.
