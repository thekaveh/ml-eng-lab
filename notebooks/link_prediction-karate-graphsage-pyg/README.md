...
This is a smaller, focused complement to `notebooks/node_classification-reddit-gnn-pyg/`. That task uses GraphSAGE for *supervised node classification* on a huge graph (Reddit2, 232k nodes). This one uses GraphSAGE as an *unsupervised edge encoder* on a tiny benchmark graph. Same architecture, different task signal (the graph's own edges vs externally-provided labels).

## 3. What's in the notebook

> **Tip:** GitHub may show "Unable to render code block" on output cells with large matplotlib PNGs. [View this notebook on nbviewer](https://nbviewer.org/github/thekaveh/ml-eng-lab/blob/main/notebooks/link_prediction-karate-graphsage-pyg/notebook.ipynb) for full rendering.
