...
- **GraphSAGE + KMeans** — train a GNN encoder (here via link prediction, same as the sibling `notebooks/link_prediction-karate-graphsage-pyg/`), then cluster the embeddings.
...
> **Tip:** GitHub may show "Unable to render code block" on output cells with large matplotlib PNGs. [View this notebook on nbviewer](https://nbviewer.org/github/thekaveh/ml-eng-lab/blob/main/notebooks/community_detection-karate-louvain-vs-gnn-pyg/notebook.ipynb) for full rendering.
